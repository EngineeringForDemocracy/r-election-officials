...
```{r glosario, include=FALSE,eval = False}
library('sf')
library(glosario)
```
...
```{r,eval = False, message=FALSE, purl=FALSE,eval = False}
...
```{r lattice, message=FALSE, purl=FALSE,eval = False}
library(lattice)
```
...
```{r first-ggplot, purl=FALSE,eval = False}
data %>%
  ggplot(aes(x = precinct)) +
  geom_bar()
```
...
```{r create-time-series, purl=FALSE,eval = False}
...
```{r time-series-points, purl=FALSE,eval = False}
hourly_counts %>%
  ggplot(aes(x = hour, y = n)) +
  geom_line() +
  geom_point()
...
```{r adding-colors, purl=FALSE,eval = False}
hourly_counts %>%
  ggplot(aes(x = hour, y = n)) +
  geom_line() +
...
the hour:
```{r change-colors, purl=FALSE,eval = False}
hourly_counts %>%
  ggplot(aes(x = hour, y = n)) +
  geom_line() +
...
```{r avg-colors, purl=FALSE,eval = False}
#calculate average
...
```{r scalepts, purl=FALSE,eval = False}
#calculate average
average <- mean(hourly_counts$n)
...
```{r barplot-wall-types-labeled,eval = False, fig.alt="Previous plot with plot title and labells added."}
hourly_counts %>%
...
```{r barplot-wall-types-labeled2,eval = False, fig.alt="Previous plot with plot title and labels added."}
hourly_counts %>%
  mutate(avg_color = ifelse(n > average, "Above", "Below")) %>%
  ggplot(aes(x = hour, y = n)) +
  geom_line() +
...
locations:
```{r boxplot, purl=FALSE,eval = False}
data %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
...
```{r boxplot-with-ticks, purl=FALSE,eval = False}
data %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
  ggplot(aes(x = location, y = hour)) +
  geom_boxplot(fill = "lightblue", color = "black") +
...
```{r boxplot-with-point, purl=FALSE,eval = False}
data %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
  ggplot(aes(x = location, y = hour)) +
  geom_boxplot(fill = "lightblue", color = "black") +
...
```{r boxplot-with-alpha, purl=FALSE,eval = False}
data %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
  ggplot(aes(x = location, y = hour)) +
  geom_boxplot(fill = "lightblue", color = "black") +
...
```{r boxplot-with-jitter, purl=FALSE,eval = False}
data %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
  ggplot(aes(x = location, y = hour)) +
  geom_boxplot(fill = "lightblue", color = "black") +
...
```{r boxplot-width-height, purl=FALSE,eval = False}
data %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
  ggplot(aes(x = location, y = hour)) +
  geom_boxplot(fill = "lightblue", color = "black") +
...
```{r boxplot-pretty, purl=FALSE,eval = False}
data %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
...
```{r boxplot-exercise-color,eval = False}
data %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
  mutate(location = str_remove(location, "LOCATION_")) %>%
  mutate(device = str_remove(device, "DEVICE_")) %>%
...
```{r percent-device-data,eval = False}
prop_device <- data %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
...
```{r barplot-device-percent,eval = False}
prop_device %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
  mutate(location = str_remove(location, "LOCATION_")) %>%
  mutate(device = str_remove(device, "DEVICE_")) %>%
...
```{r scaleandplot, eval=FALSE, purl=FALSE}
...
```{r barplot-device-hour,eval = False}
...
```{r barplot-challenge,eval = False}
...
```{r facet-theme-bw, purl=FALSE,eval = False}
prop_hour_device %>%
  mutate(device = str_remove(device, "DEVICE_")) %>%
  ggplot(aes(x = hour, y = prop, fill = device)) +
  geom_bar(stat = "identity", position = "dodge") +
...
locations:
```{r facet-location-device-fixed,eval = False}
...
```{r histogram-time-bins,eval = False}
...
```{r load-got-map-data,eval = False}
...
```{r join-data-and-map,eval = False}
ggplot() +
...
the color scale:
```{r map-daenerys-support,eval = False}
...
function:
```{r dominant-candidate-map,eval = False}
...
```{r margin-of-victory,eval = False}
...
in the lesson:
```{r ggplot-customization-advanced, purl=FALSE,eval = False}
prop_device %>%
  filter(location %in% c("LOCATION_001", "LOCATION_002", "LOCATION_003", "LOCATION_004", "LOCATION_005")) %>%
  mutate(location = str_remove(location, "LOCATION_")) %>%
  mutate(device = str_remove(device, "DEVICE_")) %>%
...
```{r ggplot-custom-themes, purl=FALSE,eval = False}
my_theme <- theme_bw() +
  theme(
    text = element_text(size = 12),
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
